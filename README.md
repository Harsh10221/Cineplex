we are on diffrent branch so make rembeber that mate
...
if a user goes on page where the language is hindi then the only check with movie id if movie id && language related show there then show the shows .

now when user clicks on an show , now add the time and showid in url and then check if this related show exist or not
movieid && language && showid && from current time period
use cannot see previous show

the ancor tag should be chagne to image tag of next js

show add

when the data is available just dont send api in tanstack query may be we need to use enabled so we can stop it
