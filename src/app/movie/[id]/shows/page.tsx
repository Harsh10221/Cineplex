...
function DateSelector() {
    ...
    // Mock Theaters Data
    const theaters = [
        { id: 1, name: "INOX: Nehru Place", times: ["10:30 AM", "01:15 PM", "04:45 PM", "09:00 PM"], amenities: ["M-Ticket", "F&B"] },
        { id: 2, name: "PVR: Select City Walk", times: ["11:00 AM", "02:30 PM", "06:00 PM", "10:30 PM"], amenities: ["Dolby Atmos", "Recliner"] },
        { id: 3, name: "Cinepolis: DLF Avenue", times: ["09:45 AM", "12:15 PM", "03:45 PM"], amenities: ["IMAX", "4DX"] },
    ];
        ...
        <div className='min-h-screen bg-gradient-to-b from-[#1a1a2e] via-[#16213e] to-[#0f3460] text-white font-sans flex flex-col'>
            ...
            <main className='flex-1 w-full max-w-5xl mx-auto px-4 pt-32 pb-12'>
                ...
                <div className="flex items-start justify-between mb-8 border-b border-white/10 pb-6">
                    <div>
                         <h1 className="text-2xl md:text-4xl font-bold text-white tracking-wide mb-2">
                            Sunny Sanskari Ki Tulsi Kumari
                        </h1>
                        <div className="flex items-center gap-3 text-sm text-gray-400">
                            <span className="px-2 py-0.5 rounded border border-gray-600 text-xs font-semibold">U/A</span>
                            <span>•</span>
                            <span>Hindi</span>
                            <span>•</span>
                    ...
                    {/* Date Display (Desktop) - Shows "26 Nov" */}
                ...
                <div className="space-y-4">
                    {theaters.map((theater) => (
                        <div key={theater.id} className="group bg-[#1f2937]/50 backdrop-blur-sm border border-white/5 rounded-2xl p-6 hover:border-white/20 transition-all duration-300 flex flex-col md:flex-row md:items-center gap-6">
                            
                            {/* Theater Info */}
                            <div className="md:w-1/3 shrink-0">
                                <h3 className="font-bold text-white text-lg tracking-wide group-hover:text-red-400 transition-colors mb-2">
                                    {theater.name}
                                </h3>
                                <div className="flex items-center gap-2 text-xs text-gray-400">
                                    {theater.amenities.map(amenity => (
                                        <span key={amenity} className="text-yellow-500/80 bg-yellow-500/10 px-2 py-0.5 rounded border border-yellow-500/20">
                                            {amenity}
                                        </span>
                                    ))}
                                </div>
